...
async function register(data, file, user) {
  ...
  return await db.brand.create(newData);
}
...
async function list(user, size, page) {
  ...
  const list = await db.brand.findAndCountAll(sqlQuery);
...
async function update(data, id, file, user) {
  let checkTestimonial = await db.brand.findOne({
    where: { id: id },
  });
  if (checkTestimonial) {
    const newData = {
